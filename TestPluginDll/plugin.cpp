...
static const char* ts3plugin_name() 
{
...
static struct PluginMenuItem* createMenuItem(enum PluginMenuType type, int id, const char* text, const char* icon) {
  auto menuItem = static_cast<struct PluginMenuItem*>(malloc( sizeof( struct PluginMenuItem ) ));
  menuItem->type = type;
  menuItem->id = id;
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) 
{
  ...
  printf("------------------------------HELLO THIS IS WERTI PLUGIN %s\n", icon_1_png);

  BEGIN_CREATE_MENUS(7);  /* IMPORTANT: Number of menu items must be correct! */
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_CLIENT_1, "Client item 1", icon_1_png);
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CLIENT, MENU_ID_CLIENT_2, "Client item 2", icon_2_png);
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_CHANNEL_1, "Channel item 1", icon_1_png);
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_CHANNEL_2, "Channel item 2", icon_2_png);
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, MENU_ID_CHANNEL_3, "Channel item 3", icon_3_png);
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL, MENU_ID_GLOBAL_1, "Global item 1", icon_1_png);
  CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL, MENU_ID_GLOBAL_2, "Global item 2", icon_2_png);
  ...
  *menuIcon = (char*)malloc(PLUGIN_MENU_BUFSZ * sizeof(char));
  _strcpy(*menuIcon, PLUGIN_MENU_BUFSZ, icon_t_png);
...
int ts3plugin_onClientPokeEvent(uint64 serverConnectionHandlerID, anyID fromClientID, const char* pokerName, const char* pokerUniqueIdentity, const char* message, int ffIgnored) {
  ...
  if (fromClientID != myID) {  /* Don't reply when source is own client */
    if (ts3Functions.requestSendPrivateTextMsg(serverConnectionHandlerID, "No.. no.. no", fromClientID, NULL) != ERROR_ok) {
      ts3Functions.logMessage("Error requesting send text message", LogLevel_ERROR, "Plugin", serverConnectionHandlerID);
    }
  }
